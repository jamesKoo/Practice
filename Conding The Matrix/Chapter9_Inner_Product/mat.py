...
def scalar_mul(M, x):
    """
        ...
        True
        """
    return Mat(M.D, {k:x*M[k] for k,v in M.f.items()})
...
def transpose(M):
    """
        ...
        True
        """
    return Mat((M.D[1], M.D[0]), {(c,r):key for (r,c), key in M.f.items()})
...
def vector_matrix_mul(v, M):
    """
        ...
        """
    assert M.D[0] == v.D
    coldict = {col:Vec(M.D[0], {row:M[row,col] for row in M.D[0]}) for col in M.D[1]}
...
def matrix_matrix_mul(A, B):
    """
        ...
        """
    assert A.D[1] == B.D[0]
    return Mat((A.D[0], B.D[1]), {(r,c):sum([A[r,i]*B[j,c] for i,j in zip(A.D[1], B.D[0])]) for r in A.D[0] for c in B.D[1]})
